...
def make_parser():
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Filename to read controller saved data",
    )
    ap.add_argument(
        "-f2",
        "--gateway_filename",
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Experiment logbook filename to get controller and gateway filenames and other details",
    )
...
def compute_experiment_results(
    node_metrics_df: pd.DataFrame, total_experiment_time: int
):
    ...
    for node_ind in desired_node_indices:
        desired_node_metrics_dict = compute_node_metrics(
            node_metrics_df, total_experiment_time, node_ind
        )
...
def main():
    ...
    if (args.controller_filename is not None) and (args.gateway_filename is not None):
        ...
        print(
            f"controller file: {controller_output_file}, \ngateway file:{gateway_output_file}"
        )
    else:
